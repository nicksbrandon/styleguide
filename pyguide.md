...
# ~~Google~~ Echobox Python Style Guide
...
```
pylint /pth/to/files/file.py
```

or to run on directory ..

```
pylint /pth/to/files/*.py
```

**note:** don't just point at folder - this fails.
 
To overide default settings first generate config file ..

```
pylint --generate-rcfile > standard.rc
```

modify thhe settings in the file ..

For example you can change the maximum length of a line ..
...
To call pylint using this new config file ..

```
pylint --rcfile=standard.rc /path/to/file/file.py
```
